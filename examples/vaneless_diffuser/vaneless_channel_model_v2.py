...
def area_fun(m, b_in, div, r_in, phi):
    return width_fun(b_in, div, m) * radius_fun(
        r_in, phi, m
    )  # Does not include the 2*pi
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------------------
    # Midline (centerline) NURBS
    # ---------------------------------------------------------------------

    z = jnp.array(
        [
            geometry["z_in"],
            geometry["z_in"] + geometry["tp_in"] * jnp.cos(geometry["phi_in"]),
            geometry["z_out"] - geometry["tp_out"] * jnp.cos(geometry["phi_out"]),
            geometry["z_out"],
        ]
    )
    ...
    r = jnp.array(
        [
            geometry["r_in"],
            geometry["r_in"] + geometry["tp_in"] * jnp.sin(geometry["phi_in"]),
            geometry["r_out"] - geometry["tp_out"] * jnp.sin(geometry["phi_out"]),
            geometry["r_out"],
        ]
    )

    # Stack to form control point matrix (2 × n_ctrl)
    ...
    # ---------------------------------------------------------------------
    # Channel height (thickness) NURBS
    # ---------------------------------------------------------------------
    ...
    channel_height = nrb.NurbsCurve(control_points=P_height, degree=3)

    # Evaluate coordinates and normals of the channel midline
    N = 100
    u_s_handle, s_total = channel_midline.reparametrize_by_arclength()
    u = u_s_handle(jnp.linspace(0.0, s_total, N))
    xy = channel_midline.get_value(u)
    n = channel_midline.get_normal(u)

    # Impose the channel height coordinate
    s_coords = jnp.linspace(P_height[0, 0], P_height[0, -1], N)
    u_x_handle = channel_height.reparametrize_by_coordinate(dim=0)
    _, b = channel_height.get_value(u_x_handle(s_coords))
    C_upper = xy + 0.5 * n * b
    C_lower = xy - 0.5 * n * b

    # Plot the channel
    fig, ax = channel_midline.plot(control_points=True)
    ax.plot(C_upper[0], C_upper[1], "orange", marker="none", label="upper wall")
    ax.plot(C_lower[0], C_lower[1], "orange", marker="none", label="lower wall")
