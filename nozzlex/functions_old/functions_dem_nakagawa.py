...
def pipeline_steady_state_1D_autonomous(
    ...
    def odefun(y):
        ...
        area, area_slope, perimeter, height = get_linear_convergent_divergent(
    ...
    if critical_flow is True: 
        ...
        while error > tol:
            ...
            raw_solution = scipy.integrate.solve_ivp(
                ...
                [0, 1],
                [1e-9, pressure_in, u_guess, enthalpy_in, 0.00],
        ...
        subsonic_solution = scipy.integrate.solve_ivp(
            lambda t, y: odefun(y)[0],
            [0, 1],
            [1e-9, pressure_in, u_possible, enthalpy_in, 0.00],
            # t_eval=np.linspace(0, convergent_length, number_of_points) if number_of_points else None,
            method="RK45",
            rtol=1e-6,
            atol=1e-6,
        ...
        subsonic_solution = postprocess_ode_autonomous(subsonic_solution.t, subsonic_solution.y, odefun)
        print("Subsonic solution completed")

        flow_rate = subsonic_solution["mass_flow"][-1]
        density = subsonic_solution["density"][-1]
        z_downstream = convergent_length*1.0001
        area_downstream, _, _, _ = get_linear_convergent_divergent(z_coordinate=z_downstream, 
                                                        ...
                                                        width=width,
                                                        type=nozzle_type)
        velocity = flow_rate/(density*area_downstream)

        # supersonic_solution = scipy.integrate.solve_ivp(
        #     lambda t, y: odefun(y)[0],
        #     [0, 1],
        #     [z_downstream, subsonic_solution["pressure"][-1]*0.99, subsonic_solution["velocity"][-1]*1.05, subsonic_solution["enthalpy"][-1]*0.99, subsonic_solution["gamma"][-1]*1.1],
        #     # t_eval=np.linspace(0, convergent_length, number_of_points) if number_of_points else None,
        #     method="RK45",
        #     rtol=1e-6,
        #     atol=1e-6,
        #     events=[stop_at_zero, stop_at_length]
        # )     
    ...
    return mass_flow, pif_iterations, subsonic_solution, supersonic_solution
