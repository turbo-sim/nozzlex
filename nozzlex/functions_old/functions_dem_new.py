...
def forward_diff(f, x, h=1e-5):
    """
    Compute the derivative of a function f at point x
    ...
    Returns:
        float : Approximation of derivative f'(x)
    """
    return (f(x + h) - f(x)) / h
...
def pipeline_steady_state_1D_autonomous(
    ...
    def odefun(y):
        ...
        try:
            ...
            state = fluid.get_state(bpy.HmassP_INPUTS, h, p)
            ...
            area, area_slope, perimeter, radius = get_linear_convergent_divergent(
                length=z, convergent_length=convergent_length, divergent_length=divergent_length, radius_in=radius_in, radius_throat=radius_throat,
